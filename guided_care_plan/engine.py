...
def _q_header(title: str):
    """Question title with step prefix, kept simple to allow CSS to style."""
    st.markdown(f"**{title}**")

def _q_info_below(bullets):
    """Compact 'Why we ask' block placed BELOW nav for good mobile ergonomics."""
    if not bullets:
        return
...
QUESTIONS = [
    (
        "funding_confidence",
        "Step 1: How would you describe your financial situation when it comes to paying for care?",
    ...
    (
        "cognition_level",
        "Step 2: How would you rate your memory and thinking in daily life?",
    ...
    (
        "adl_dependency",
        "Step 3: How well can you manage everyday activities like bathing, dressing, or preparing meals on your own?",
    ...
    (
        "meds_complexity",
        "Step 4: Do you take medications, and how manageable is the routine?",
    ...
    (
        ...
        "Step 5: How much regular support do you have from a caregiver or family member?",
        [
        ...
        ["This shows whether consistent caregiver help is available.", "Strong support can offset higher daily needs."],
    ),
    (
        "mobility",
        "Step 6: How do you usually get around?",
        ["I walk easily", "I use a cane", "I use a walker", "I use a wheelchair"],
        ["We mean typical movement at home and outside."],
    ),
    (
        "social_isolation",
        "Step 7: How often do you connect with friends, family, or activities?",
        ["Frequent contact", "Occasional contact", "Rarely see others", "Often alone"],
        None,
    ),
    (
        "geographic_access",
        "Step 8: How accessible are services like pharmacies, grocery stores, and doctors from your home?",
        ["Very easy", "Moderate", "Difficult", "Very difficult"],
        ["This helps plan logistics and support for errands, meds, and visits."],
    ),
    (
        "chronic_conditions",
        "Step 9: Do you have any ongoing health conditions? Select all that apply.",
    ...
    (
        "home_setup_safety",
        "Step 10: How safe and manageable is your home for daily living as you age?",
    ...
    (
        "recent_fall",
        "Step 11: Has there been a fall in the last 6 months?",
        ["Yes", "No", "Not sure"],
        ["Recent falls increase the need for supervision or home changes."],
    ),
    (
        "move_willingness",
        "Step 12: If care is recommended, how open are you to changing where care happens?",
        [
...
# ------------------------------------------------------------------------------
def render():
    """External entry-point: boot session and run flow controller."""
...
def _run_flow():
    ...
    # ----------------------------------------------------------
    # Step 0 – Audiencing (hero is rendered in app.py)
    # ----------------------------------------------------------
    if step == 0:
        ...
        if audience == "One person":
            care_context["person_a_name"] = st.text_input(
                "What's their name?", value=care_context.get("person_a_name", "")
            )
            care_context["person_b_name"] = None

        elif audience == "Two people":
            care_context["person_a_name"] = st.text_input(
                "First person's name", value=care_context.get("person_a_name", "")
            )
            care_context["person_b_name"] = st.text_input(
                "Second person's name", value=care_context.get("person_b_name", "")
            )
            ...
            care_context["person_a_name"] = st.text_input(
                "Person A name", value=care_context.get("person_a_name", "")
            )
            care_context["person_b_name"] = (
                st.text_input(
                    "Person B name (if applicable)",
                    value=care_context.get("person_b_name", ""),
                )
                if who == "Two"
                else None
            )

        # NAV for step 0 (Next only)
        st.markdown('<div class="scn-nav-row">', unsafe_allow_html=True)
        nav_only = st.columns([1])
        with nav_only[0]:
            if st.button("Next", type="primary", key="next_0"):
    ...
    if 1 <= step <= len(QUESTIONS):
        ...
        if key == "chronic_conditions":
            sel = st.multiselect(
                ...
                QUESTIONS[8][2],
                default=care_context.get("chronic_conditions", []),
                key="cc_multi",
            )
        ...
        # --- NAV: Back / Next (forced horizontal via .scn-nav-row) ---
        st.markdown('<div class="scn-nav-row">', unsafe_allow_html=True)
        nav_c1, nav_c2 = st.columns([1, 1])
        with nav_c1:
            if st.button("Back", type="secondary", key=f"back_{step}"):
                st.session_state.planner_step = max(0, step - 1)
                st.rerun()
        with nav_c2:
            next_disabled = (
                False if key == "chronic_conditions" else care_context["flags"].get(key) is None
            )
            if st.button("Next", disabled=next_disabled, type="primary", key=f"next_{step}"):
